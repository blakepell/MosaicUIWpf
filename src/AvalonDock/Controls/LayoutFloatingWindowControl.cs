...
namespace AvalonDock.Controls
{
    ...
    public abstract class LayoutFloatingWindowControl : Window, ILayoutControl
    {
        ...
        private bool _isTotalMarginSet;
        ...
        protected void OnIsDraggingChanged(DependencyPropertyChangedEventArgs e)
        {
            if ((bool)e.NewValue)
            {
                CaptureMouse();
        ...
        {
            get => (bool)GetValue(OwnedByDockingManagerWindowProperty);
        ...
        {
            get => (Thickness)GetValue(TotalMarginProperty);
            ...
            if (!_isTotalMarginSet)
            {
                ...
                if (_isTotalMarginSet)
                {
                    ...
                    foreach (var content in contents)
                    {
                        ...
                        if ((Model?.Root?.Manager?.AutoWindowSizeWhenOpened).GetValueOrDefault())
                        {
        ...
        private void UpdateMaximizedState(bool isMaximized)
        {
            ...
            else if (!AllowMinimize || WindowState != WindowState.Minimized)
            {
