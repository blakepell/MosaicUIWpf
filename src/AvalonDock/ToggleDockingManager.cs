...
public class ToggleDockingManager : DockingManager
{
    ...
    /// </summary>
    private readonly ToggleLayoutEngine _layoutEngine = new();
    ...
    private readonly Dictionary<IToolbox, LayoutAnchorable> _toolboxToAnchorable = new();
    ...
    protected override void OnThemeChanged(DependencyPropertyChangedEventArgs e)
    {
        ...
        if (IsLoaded)
        {
            ...
            // the same template structure (for example, VS2015 Dark and Light).
            Dispatcher.BeginInvoke(
                DispatcherPriority.Loaded,
                new Action(ReapplyThemeStyles));
        }
    }
    ...
    private void ReapplyThemeStyles()
    {
        ...
        Dispatcher.BeginInvoke(
            DispatcherPriority.Loaded,
            new Action(UpdatePinButtonsToMinimize));
    }
    ...
    private void UpdatePinButtonsToMinimize()
    {
        foreach (var title in FindVisualChildren<AnchorablePaneTitle>(this))
        {
            ...
            foreach (var btn in FindVisualChildren<Button>(title))
            {
                if (btn.Name == "PART_AutoHidePin")
                {
                    btn.ToolTip = "Minimize";
                    if (btn.Content is not Border border)
                    {
            ...
            if (grid != null && !grid.Children.OfType<Button>()
                    .Any(b => b.Name == "PART_ToggleMenu"))
            {
                ...
                var autoHideBtn = grid.Children.OfType<Button>()
    ...
    internal ContextMenu BuildToggleContextMenu(LayoutAnchorable anchorable)
    {
        ...
        {
            var zoneLabel = Regex.Replace(zone.ToString(), "(\\B[A-Z])", " $1");
            var z = zone;
    ...
    {
        _layoutEngine.EnsureBottomFullWidth(Layout);
    }
    ...
    {
        _layoutEngine.EnsureSidesFullHeight(Layout);
    }
    ...
    {
        if (anchorable.Parent is not LayoutAnchorGroup parentGroup)
        {
            return;
        }
    ...
    {
        if (anchorable.Parent is not LayoutAnchorablePane parentPane)
        {
            return;
        }
    ...
    private void SetupToggleDockButtonBars()
    {
        ...
        AutomationProperties.SetAutomationId(_leftTopBar, "ToggleDockBar_LeftTop");
        ...
        AutomationProperties.SetAutomationId(_leftBottomBar, "ToggleDockBar_LeftBottom");
        ...
        AutomationProperties.SetAutomationId(_rightTopBar, "ToggleDockBar_RightTop");
        ...
        AutomationProperties.SetAutomationId(_rightBottomBar, "ToggleDockBar_RightBottom");
        ...
        AutomationProperties.SetAutomationId(_bottomLeftBar, "ToggleDockBar_BottomLeft");
        ...
        AutomationProperties.SetAutomationId(_bottomRightBar, "ToggleDockBar_BottomRight");
    ...
    private Button CreateShowHiddenButton()
    {
        ...
        _showHiddenButton.SetResourceReference(ForegroundProperty, ToggleDockButton.ForegroundBrushKey);
    ...
    private static ControlTemplate CreateShowHiddenButtonTemplate()
    {
        ...
        pathFactory.SetValue(WidthProperty, 12.0);
        pathFactory.SetValue(HeightProperty, 12.0);
        pathFactory.SetValue(HorizontalAlignmentProperty, HorizontalAlignment.Center);
        pathFactory.SetValue(VerticalAlignmentProperty, VerticalAlignment.Center);
    ...
    {
        if (!_toolboxToAnchorable.TryAdd(toolbox, anchorable))
        {
