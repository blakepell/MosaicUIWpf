...
namespace Mosaic.UI.Wpf.AvalonDock.Controls
{
    ...
    internal sealed class FullWeakDictionary<TKey, TValue>
        where TKey : class
        where TValue : class
    {
        ...
        public void SetValue(TKey key, TValue value)
        {
            ...
            if (index >= 0)
            {
                _values[index] = new WeakReference<TValue>(value);
            }
            else
            {
                _values.Add(new WeakReference<TValue>(value));
        ...
        public bool GetValue(TKey key, out TValue value)
        {
            CollectGarbage();
            var index = FindKeyIndex(key);
            if (index < 0 || !_values[index].TryGetTarget(out value))
            {
                value = default!;
                return false;
            }
        ...
        private void CollectGarbage()
        {
            for (var index = _keys.Count - 1; index >= 0; index--)
            {
                if (!_keys[index].TryGetTarget(out _) ||
                    !_values[index].TryGetTarget(out _))
                {
