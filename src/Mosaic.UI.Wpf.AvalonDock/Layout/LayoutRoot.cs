...
namespace Mosaic.UI.Wpf.AvalonDock.Layout
{
    ...
    [Serializable]
    public class LayoutRoot : LayoutElement, ILayoutContainer, ILayoutRoot, ISerializableLayoutRoot
    {
        ...
        public void CollectGarbage()
        {
            ...
            {
                exitFlag = true;
                var descendants = this.Descendents().ToArray();
                var previousContainerElements = descendants.OfType<ILayoutPreviousContainer>().ToArray();
                var layoutContents = descendants.OfType<LayoutContent>().ToArray();
                var mainDocumentPaneCount = descendants
                    .OfType<LayoutDocumentPane>()
                    .Count(pane => pane.FindParent<LayoutDocumentFloatingWindow>() == null);

                // for each content that references via PreviousContainer a disconnected Pane set the property to null
                foreach (var content in previousContainerElements.Where(c => c.PreviousContainer != null &&
                ...
                foreach (var emptyPane in descendants.OfType<ILayoutPane>().Where(p => p.ChildrenCount == 0))
                {
                    ...
                    if (emptyPane is LayoutDocumentPane &&
                         ...
                         mainDocumentPaneCount == 1)
                    {
                        continue;
                    }

                    // ...if this empty pane is not referenced by anyone, then remove it from its parent container
                    if (!previousContainerElements.Any(c => c.PreviousContainer == emptyPane))
                    {
        ...
        {
            var removedContents = element.Descendents().OfType<LayoutContent>().ToArray();
            if (ReferenceEquals(element, LastFocusedDocument) || removedContents.Any(c => c == LastFocusedDocument))
            {
                LastFocusedDocument = null;
            }

            if (ReferenceEquals(element, ActiveContent) || removedContents.Any(c => c == ActiveContent))
            {
                ActiveContent = null;
            }
        ...
        /// <inheritdoc/>
        IEnumerable<ISerializableLayoutElement> ISerializableLayoutRoot.Descendents()
            => this.Descendents().OfType<ISerializableLayoutElement>();
    }
}
