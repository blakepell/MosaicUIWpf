...
| [FileCard](./docs/FileCard.md) | A clickable card for a single file that shows the OS icon for the file type alongside the file name and formatted size, tooltips the full path, falls back to an error glyph when the file is missing, and optionally tints itself with the icon's dominant color. Raises Click and executes a Command with the file path. |
