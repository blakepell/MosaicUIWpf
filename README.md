...
| [DateSpinner](./docs/DateSpinner.md) | A date picker that dials in a date with three coordinated month, day and year wheels instead of a calendar grid, in the style of the WinUI 3 date picker. Culture-aware field order, nullable selection, range limits with boundary filtering, and explicit or immediate commit. |
| [DayCalendarView](./docs/DayCalendarView.md) | Displays a selected day as a scrollable, continuously positioned timeline with overlap columns, commands, and snapped event dragging. |
...
| [EnumComboBox](./docs/EnumComboBox.md) | A ComboBox that populates itself from an enum type, displaying friendly names (EnumDisplayName → Description → member name) while exposing the actual enum value through SelectedValue. |
| [ExecutionControl](./docs/ExecutionControl.md) | A transport style control that exposes Play, Pause and Stop buttons suitable for a tool bar. Each button binds to its own command and is disabled and visually muted when that command reports it cannot execute. |
...
| [FileCard](./docs/FileCard.md) | A clickable card for a single file that shows the OS icon for the file type alongside the file name and formatted size, tooltips the full path, falls back to an error glyph when the file is missing, and optionally tints itself with the icon's dominant color. Raises Click, executes a Command with the file path, and can optionally open the file through the OS shell. |
...
| [FontStyleComboBox](./docs/FontStyleComboBox.md) | A ComboBox that lists the font styles (Normal, Italic, Oblique), rendering each entry in the style it names and exposing the selection as either a FontStyle or a style name. |
...
| [LineGraph](./docs/LineGraph.md) | A line graph that plots multiple series of time stamped values, each with its own colour and caption, over minutes, hours, days, weeks, months or years. Optional title, theme-aware axes, and click events for lines and points. |
